...
- **Accuracy, from a one-line description, with no tuning:** `jgrep` finds SMS spam with F1 0.97 (n=120), where a 17-term keyword regex scores 0.72 over all 5,574 messages; `jsort` ranks review sentiment with AUC 1.00 (n=80); `jtag` labels AG News four ways with 50% accuracy (n=20).
